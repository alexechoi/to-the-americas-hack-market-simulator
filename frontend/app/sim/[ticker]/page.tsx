...
export default function TickerSimPage() {
  const params = useParams<{ ticker: string }>();
  const ticker = (params.ticker ?? "").toUpperCase();

  // Spawn the exchange for this ticker once on mount.
  const spawnedRef = useRef(false);
  useEffect(() => {
    if (!ticker || spawnedRef.current) return;
    spawnedRef.current = true;
    spawnExchange(ticker).catch((err) =>
  ...
  const displayTicker = exchangeState?.ticker ?? ticker;
  const tickerName = exchangeState?.name ?? "";
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[var(--color-bg)] text-[var(--color-fg)]">
      ...
      <header className="flex shrink-0 items-center gap-6 border-b border-[var(--color-line)] bg-[var(--color-surface)] px-5 py-2.5">
        ...
        <div className="flex items-baseline gap-3">
          <h1 className="text-base font-semibold tracking-tight">{displayTicker}</h1>
          <span className="hidden font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--color-fg-faint)] md:inline">
            {tickerName ? `${tickerName} · single-name` : "single-name"}
          </span>
        </div>
